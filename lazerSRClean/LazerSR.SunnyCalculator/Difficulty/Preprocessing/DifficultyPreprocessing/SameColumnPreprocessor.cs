...
namespace osu.Game.Rulesets.Mania.Difficulty.Preprocessing.DifficultyPreprocessing
{
    public static class SameColumnPreprocessor
    {
        ...
        // jack_nerf_coefficient -> SunnyConstants.JackNerfCoefficient
        ...
        {
            List<ManiaDifficultyHitObject>[] notesByColumn = data.AllNotes.First().PerColumnObjects;
        ...
        {
            double timingDeviation = Math.Abs(deltaTimeSeconds - SunnyConstants.JackOptimalInterval);
            double nerfBase = jack_nerf_base + timingDeviation;
            double nerfValue = SunnyConstants.JackNerfCoefficient * Math.Pow(nerfBase, jack_nerf_power);
