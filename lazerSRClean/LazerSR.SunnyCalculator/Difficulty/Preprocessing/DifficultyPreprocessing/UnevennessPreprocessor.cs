...
namespace osu.Game.Rulesets.Mania.Difficulty.Preprocessing.DifficultyPreprocessing
{
    public static class UnevennessPreprocessor
    {
        ...
        private static double[][] computeTimingDeltasByColumn(ManiaDifficultyData data)
        {
            ...
            List<ManiaDifficultyHitObject>[] columnNoteArray = data.AllNotes.First().PerColumnObjects;

            for (int column = 0; column < data.KeyCount; column++)
            {
                double[] columnDeltas = new double[timePointCount];
                List<ManiaDifficultyHitObject> columnNotes = columnNoteArray[column];
        ...
        {
            if (timingDifference < SunnyConstants.UnevennessLowThreshold)
            {
                // Very small differences => mild penalty with timing component
                return Math.Min(SunnyConstants.UnevennessFloor + 0.5 * Math.Max(leftDelta, rightDelta), 1.0);
            }

            if (timingDifference < SunnyConstants.UnevennessHighThreshold)
            {
                // Moderate differences => variable penalty. Slope and intercept are derived from the
                // floor and the two thresholds so this branch stays continuous with the ones around it.
                return Math.Min(SunnyConstants.UnevennessIntercept + SunnyConstants.UnevennessSlope * timingDifference
                                + 0.5 * Math.Max(leftDelta, rightDelta), 1.0);
            }
