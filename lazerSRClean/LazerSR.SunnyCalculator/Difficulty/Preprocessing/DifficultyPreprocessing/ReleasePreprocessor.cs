    ...
    public static class ReleasePreprocessor
    {
        ...
        private static double[] calculateIndividualReleaseDifficulties(ManiaDifficultyData data)
        {
            ...
            for (int i = 0; i < data.LongNoteTails.Count; i++)
            {
                ...
                double holdDuration = Math.Abs(tailNote.EndTime - tailNote.StartTime - SunnyConstants.ReleaseIdealSlack);
                ...
                {
                    releaseToNextNote = Math.Abs(nextNoteInColumn.StartTime - tailNote.EndTime - SunnyConstants.ReleaseIdealSlack);
                }
                else
                {
                ...
                double lh = DifficultyValueUtils.Logistic(holdDifficultyComponent, 0.75, 5.0);
                double lt = DifficultyValueUtils.Logistic(timingDifficultyComponent, 0.75, 5.0);
        ...
        private static void processConsecutiveLongNoteTails(ManiaDifficultyData data, double[] releaseDifficulties, double[] releaseFactorBase, int timePoints)
        {
            for (int i = 0; i + 1 < data.LongNoteTails.Count; i++)
            {
                ...
                double difficultyMultiplier = 1.0 + SunnyConstants.ReleaseMultiplier * (releaseDifficulties[i] + releaseDifficulties[i + 1]);
