    ...
    public class LongNoteDensityPreprocessor
    {
        ...
        public static void ProcessAndAssign(ManiaDifficultyData data)
        {
            ...
            for (int i = 0; i < longNoteCount; i++)
            {
                ...
                double earlyPoint = Math.Min(longNote.StartTime + SunnyConstants.LnDensityEarlyMs, longNote.EndTime);
                double midPoint = Math.Min(longNote.StartTime + SunnyConstants.LnDensityMidMs, longNote.EndTime);

                // Add density at different points of the long note
                eventTimes[eventPosition] = earlyPoint;
                densityDeltas[eventPosition++] = SunnyConstants.LnDensityUp; // Strong initial impact

                eventTimes[eventPosition] = midPoint;
                densityDeltas[eventPosition++] = SunnyConstants.LnDensityMid; // Slight reduction

                eventTimes[eventPosition] = longNote.EndTime;
                densityDeltas[eventPosition++] = SunnyConstants.LnDensityEnd; // Full removal at the end (derived)
            }
        ...
        private static LongNoteDensityData processDensityEvents(double[] eventTimes, double[] densityDeltas, int eventCount)
        {
            ...
            while (eventIndex < eventCount)
            {
                ...
                double clampedDensity = Math.Min(currentDensity, SunnyConstants.LnDensityClampBase + SunnyConstants.LnDensityClampSlope * currentDensity);
