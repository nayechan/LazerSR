...
namespace osu.Game.Rulesets.Mania.Difficulty.Evaluators
{
    public class StrainEvaluator
    {
        public static double EvaluateDifficultyOf(ManiaDifficultyHitObject current)
        {
            ...
            double clampedSameColumn = Math.Min(sameColumn, SunnyConstants.JackKnee + 0.85 * sameColumn);
            ...
            double firstComponent = SunnyConstants.MixFirst * Math.Pow(unevennessSameColumnComponent, SunnyConstants.MixExponent);

            // Combine unevenness with pressing intensity and release difficulty
            double releaseComponent = release * DifficultyValueUtils.Smoothstep(activeKeyCount, 0, 4) * SunnyConstants.ReleaseDensityNorm / (localNoteCount + 8.0);
            double unevennessPressingReleaseComponent = Math.Pow(unevenness, 2.0 / 3.0) * (SunnyConstants.PressingWeight * pressingIntensity + releaseComponent);
            double secondComponent = SunnyConstants.MixSecond * Math.Pow(unevennessPressingReleaseComponent, SunnyConstants.MixExponent);

            // Main strain difficulty combining both components
            double totalStrainDifficulty = Math.Pow(firstComponent + secondComponent, SunnyConstants.MixOuterExponent);
            ...
            double finalStrain = SunnyConstants.TwistFirst * Math.Sqrt(totalStrainDifficulty) * poweredTwist + totalStrainDifficulty * SunnyConstants.TwistSecond;
