...
// The ONLY divergence from the original is the single line that obtains the StarDifficulty bindable —
// `difficultyCache.GetBindableDifficulty(...)` is swapped for `SunnyCalculatorRunner.GetBindableDifficulty(...)`,
// plus the internal `WedgeBackground` is swapped for `SunnyWedgeBackground` (cloned because the original is internal).
...
public partial class SunnyDifficultyDisplay : CompositeDrawable
{
    ...
    private void load(OverlayColourProvider colourProvider)
    {
        ...
        {
            // === clone divergence: WedgeBackground is internal sealed in osu.Game, use the public clone ===
    ...
    private void updateDisplay()
    {
        ...
        starRatingDisplay.Current = (Bindable<StarDifficulty>)SunnyCalculatorRunner.GetBindableDifficulty(beatmap.Value, ruleset.Value, mods.Value, cancellationSource.Token);
        // === end divergence ===
