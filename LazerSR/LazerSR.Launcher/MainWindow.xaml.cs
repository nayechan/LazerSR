...
public partial class MainWindow : Window
{
    ...
    {
        if (_osuProcess == null) { StatusTextBlock.Text = "No osu! process running."; return; }
        DisconnectPipe();
        StartPipeClient(_osuProcess.Id);
    }
    ...
    private async void FeatureToggle_Changed(object sender, RoutedEventArgs e)
    {
        ...
        if (sender is not ToggleButton btn || btn.Tag is not string featureName) return;
        bool on = btn.IsChecked == true;
        await _pipeClient.SendAsync($"feature:{featureName}:{(on ? "on" : "off")}");
    }
    ...
    {
        bool connected = status == PipeStatus.Connected;
    ...
    private IEnumerable<ToggleButton> GetToggles() =>
        new[] { SunnySRToggle, DanToggle, GraphToggle, HoneyToggle, MsdToggle };
