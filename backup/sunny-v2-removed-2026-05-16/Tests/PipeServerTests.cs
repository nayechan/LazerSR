...
[TestFixture]
public sealed class PipeServerFeatureCommandTests
{
    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
    ...
    [SetUp]
    public void SetUp() => SunnyState.ResetForTesting();

    [Test]
    public async Task FeatureDanOn_SetsDanEnabled()
    {
        using var client = await ConnectAsync();
        await client.Reader.ReadLineAsync(); // "connected"

        await client.Writer.WriteLineAsync("feature:dan:on");
        await client.Writer.FlushAsync();

        await PollAsync(() => SunnyState.DanEnabled, expectedValue: true);
